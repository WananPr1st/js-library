    ...
    .run([
      ...
      function($templateCache) {
        $templateCache.put('template/treeGrid/treeGrid.html',
          ...
          "         <div ng-if=\"col.headerTemplate\"                            compile=\"col.headerTemplate\"></div>\n" +
          "         <div ng-if=\"!col.headerTemplate\">{{col.displayName || col.field}}</div>\n" +
          "</th>\n" +
          "     </tr>\n" +
          "   </thead>\n" +
          "   <tbody>\n" +
      ...
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          scope.$watch(attrs.compile, function(new_val) {
            var cellTemplate = '';
            // 兼容模板文件和string两种形式
            if (/(\.html)$/g.test(new_val)) {
              cellTemplate = $templateCache.get(new_val);
              if (!cellTemplate) {
                var tpl = $.ajax({
                  type: 'GET',
                  url: new_val,
    ...
    function($timeout,
      ...
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          scope.$watch('colDefs', function(newValue, oldValue) {
            scope.colDefinitions = scope.colDefs;
          }, true);
          ...
          for_each_branch(function(b, level) {
            b.level = level;
            // 默认关闭 change yanlingling
            return b.expanded = false; //b.level < expand_level;
          });
