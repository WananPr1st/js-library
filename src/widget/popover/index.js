...
+function ($) {
  ...
  Popover.prototype.getOptions = function (options) {
    ...
    // 先mock一下，默认禁止animate，防止$tip进入时触发消失动画
    // 回头再改
  ...
  Popover.prototype.getUID = function (prefix) {
  ...
  Popover.prototype.tip = function () {
    ...
    // 扩展一下tip，当离开$element并且移动到tip的时候，不移除tip
    // 当两者都离开的时候再移除tip
    // click监听事件除外
    if(this.options.trigger === 'hover'){
      this.$tip.on('mouseenter.' + this.type, $.proxy(this.enter, this));
      this.$tip.on('mouseleave.' + this.type, $.proxy(this.leave, this));
    }
    if (this.options.color) this.$tip.addClass('popover-' + this.options.color);
    return this.$tip
  }
